...
namespace utils::pmr {
    ...
    /// (In other words, `deallocate` is a no-op.)
    ...
    ///
    /// Internally, `arena_allocator` only stores a pointer to an implementation-defined control structure
    ...
    /// If `Sync` is true, updates are performed atomically.
    ...
    ///
    /// All instances of this class satisfy [<i>BufferAllocator</i>](BufferAllocator.html) and `resource_allocator`.
    /// Instances with `Sync` equal to `true` also satisfy [<i>ConvertibleAllocator</i>](BufferAllocator.html).
