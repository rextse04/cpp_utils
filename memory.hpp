...
namespace utils {
    ...
    /// @param end: Pointer to the end of the storage such that [ `ptr`, `end` ) is a valid range.
    /// @returns If it is possible to fit a storage of `size` in [ `ptr`, `end` )
    /// aligned to `alignment`, a pointer to the first byte of such storage; `nullptr` otherwise.
    ...
    /// @brief A trivial class type that is aligned to `Alignment`.
    /// @remark It is ill-formed if `Alignment` is invalid or unsupported.
