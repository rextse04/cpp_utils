...
namespace utils {
    /// @brief Thin wrapper around a void pointer that acts like a `this` pointer.
    ///
    /// This class allows the `dyn_method` template to specialize for non-static dynamic methods.
    ...
    public:
        ...
        constexpr T* operator+(integer_alias::size_t offset) noexcept { return obj_ + offset; }
        constexpr T* operator-(integer_alias::size_t offset) noexcept { return obj_ - offset; }
    ...
    /// @brief A composition class which glues multiple `ivtable`-s together.
    template <interface... Is>
    ...
    /// @brief Denotes an implementer of each of `Is`.
    ///
...
#define UTILS_DYN_END ,\
    ...
    /// @brief Convenience wrapper class for deleters
    ///
    ...
    public:
        ...
        ///
        /// Also sets `other.ptr_` to `nullptr`.
        ...
        /// @brief Construction from a `dptr` with a stricter interface.
        ///
        ...
        /// @brief Construction from a `dptr` with a stricter interface, while constructing a separate deleter.
        ///
        /// Copies `ptr_` and the required subset of `ivtables_` from `other`.
        /// If `other` is moved from, `other.ptr_` is set to `nullptr`.
        ...
        /// @brief Copy assignment from a `dptr` with a stricter interface.
        ///
        ...
        /// @brief Move assignment from a `dptr` with a stricter interface.
        ///
        /// If the pointer is not borrowed and `ptr_` is not `nullptr`, calls `destroy_and_delete()`.
        ...
        ///
        /// Only enabled if the underlying object is an array (i.e. if `is_array` is true).
        /// @note Let `T` be the type of the underlying object.
        /// The behavior is undefined if performing `(T*)ptr_ + idx` results in an invalid pointer value.
        ...
        /// @brief Destroy the underlying object (but does not call deleter on `ptr_`).
        ///
        /// The behavior is undefined if `ptr_ == nullptr`.
        /// Using the destroyed object (which occurs if you call a non-static `dyn_method`) afterwards is undefined.
        /// @remark Dangerous operation, as destroying an object twice is undefined.
        ...
        /// @}
        /// @brief Releases ownership of `ptr_` and returns it.
        ...
        /// @brief Returns an equivalent `dptr` with the default deleter.
